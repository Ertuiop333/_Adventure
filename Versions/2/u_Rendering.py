...
def __write_render_object_to_screen_data(object: Components.Renderer):
    for pixel in object.sprite.pixels:
        newX = pixel.position.x + object.master_entity.position.x - camera_pos.x
        newY = pixel.position.y + object.master_entity.position.y - camera_pos.y
        if (
            ...
            Terminal.screen_data[(pixel.position.x + object.master_entity.position.x - camera_pos.x)][
                (pixel.position.y + object.master_entity.position.y - camera_pos.y)
            ] = pixel.character
...
def write_render_queue_to_screen_data(camera_position):
    global camera_pos
